...
mkdir -p backup_before_mc9d2_override_review_ui
...
do
  if [ -f "$f" ]; then
    cp "$f" backup_before_mc9d2_override_review_ui/$(basename "$f")
  fi
done

cat > public/csvb-question-overrides-admin.js <<'JS'
// public/csvb-question-overrides-admin.js
...
const scriptTag = '<script src="./csvb-question-overrides-admin.js?v=20260430_1"></script>';

if (!html.includes("csvb-question-overrides-admin.js")) {
...
if (fs.existsSync(sw)) {
  ...
  if (/const CACHE_VERSION = "[^"]+";/.test(s)) {
    s = s.replace(
      /const CACHE_VERSION = "[^"]+";/,
      'const CACHE_VERSION = "v27-mc9d2-question-overrides-admin";'
    );
  }

  if (!s.includes('"./csvb-question-overrides-admin.js"')) {
...
fs.writeFileSync(
  "public/MC9D2_QUESTION_OVERRIDES_ADMIN_APPLIED.txt",
  "MC-9D2 applied: Superuser question override review UI added. No SQL/auth/RLS changes.\\n",
  "utf8"
);

console.log("DONE: MC-9D2 Question Override Review UI applied.");
NODE

echo "DONE: MC-9D2 completed."
