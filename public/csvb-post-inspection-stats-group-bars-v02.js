// C.S.V. BEACON - Post-Inspection Stats Group Bars v02
// Display-only helper. Adds metric selector to group bar charts.
// No stored data, calculations, filters, drilldowns, exports, auth, Supabase, device or offline logic is changed.
(() => {
  "use strict";

  const BUILD = "POST-INSPECTION-STATS-GROUP-BARS-V02-20260601";
  ...
  const METRICS = [
    ["selected_count", "Selected observation count"],
    ["negative", "Negative count"],
    ["largely", "Largely count"],
    ["positive", "Positive count"],
    ["avg_negative", "Avg Negative / Insp."],
    ["avg_largely", "Avg Largely / Insp."],
    ["avg_positive", "Avg Positive / Insp."]
  ];
  const COLORS = ["#295eb0", "#6d28d9", "#0f766e", "#be185d", "#b36b00", "#8a4b00", "#c62828", "#16803a", "#0369a1", "#9333ea", "#0d9488", "#db2777", "#ca8a04", "#475569"];

  const SECTIONS = [
    { id: "category", title: "By Category / Designation", tbody: "byCategoryTbody", cols: { negative: 1, largely: 2, positive: 3, avg_negative: 4, avg_largely: 5, avg_positive: 6 }, panel: "csvbStatsByCategoryBarsPanelV01" },
    { id: "ocimf", title: "By OCIMF / Inspecting Company", tbody: "byOcimfTbody", cols: { negative: 2, largely: 3, positive: 4, avg_negative: 5, avg_largely: 6, avg_positive: 7 }, panel: "csvbStatsByOcimfBarsPanelV01" },
    { id: "inspector", title: "By Inspector / Auditor", tbody: "byInspectorTbody", cols: { negative: 2, largely: 3, positive: 4, avg_negative: 5, avg_largely: 6, avg_positive: 7 }, panel: "csvbStatsByInspectorBarsPanelV01" },
    { id: "inspectorCompany", title: "By Inspector / Auditor Company", tbody: "byInspectorCompanyTbody", cols: { negative: 2, largely: 3, positive: 4, avg_negative: 5, avg_largely: 6, avg_positive: 7 }, panel: "csvbStatsByInspectorCompanyBarsPanelV01" }
  ];

  const state = Object.fromEntries(SECTIONS.map((s) => [s.id, { display: "count_pct", limit: 15, metric: "selected_count", types: new Set(["negative", "largely", "positive"]) }]));
  ...
  const isAvgMetric = (m) => String(m || "").startsWith("avg_");
  const metricLabel = (m) => METRICS.find((x) => x[0] === m)?.[1] || m;

  function valueFor(raw, st) {
    if (st.metric === "selected_count") {
      return (st.types.has("negative") ? raw.negative : 0) + (st.types.has("largely") ? raw.largely : 0) + (st.types.has("positive") ? raw.positive : 0);
    }
    return Number(raw[st.metric] || 0);
  }

  function displayValue(item, total, st) {
    if (isAvgMetric(st.metric)) return Number(item.value || 0).toFixed(2);
  ...
  function injectStyle() {
    if (document.getElementById("csvbGroupBarsV02Style")) return;
    const style = document.createElement("style");
    style.id = "csvbGroupBarsV02Style";
    style.textContent = `
      ...
      @media(max-width:1200px){.csvb-group-bars-controls{grid-template-columns:1fr}.csvb-group-bars{gap:10px}.csvb-group-bar-item{width:84px}.csvb-group-bar-label{width:84px}}
    `;
    document.head.appendChild(style);
  }

  function rawRows(sec) {
    ...
    return Array.from(tbody.querySelectorAll("tr")).map((tr) => {
      ...
      return {
        label,
        negative: num(cells[sec.cols.negative]?.textContent || ""),
        largely: num(cells[sec.cols.largely]?.textContent || ""),
        positive: num(cells[sec.cols.positive]?.textContent || ""),
        avg_negative: num(cells[sec.cols.avg_negative]?.textContent || ""),
        avg_largely: num(cells[sec.cols.avg_largely]?.textContent || ""),
        avg_positive: num(cells[sec.cols.avg_positive]?.textContent || "")
      };
    }).filter(Boolean);
  }
  ...
  function rowsFor(sec) {
    ...
    return rawRows(sec).map((r) => ({ ...r, value: valueFor(r, st) })).filter((x) => x.value > 0).sort((a,b) => b.value - a.value || a.label.localeCompare(b.label)).slice(0, Number(st.limit || 15));
  }
  ...
  function barsHtml(sec) {
    ...
    const metricText = metricLabel(st.metric);
    const noteTotal = isAvgMetric(st.metric) ? `Metric shown: ${esc(metricText)}.` : `Total displayed observations: ${esc(total)}.`;
    return `<div class="csvb-group-bars-wrap"><div class="csvb-group-bars">${bars}</div></div><div class="csvb-group-legend">${legend}</div><div class="csvb-group-note">${noteTotal} Highest: ${esc(data[0].label)} (${esc(displayValue(data[0],total,st))}).</div>`;
  }
  ...
  function onChange(e) {
    ...
    if (t.matches("select[data-csvb-group-metric='1']")) st.metric = String(t.value || "selected_count");
