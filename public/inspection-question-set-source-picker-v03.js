// public/inspection-question-set-source-picker-v03.js
// C.S.V. BEACON - Assurance Question Set Source Picker v03
// Uses csvb_assurance_source_questions_for_me() to load SIRE + RISQ rows.

(() => {
  "use strict";

  const BUILD = "ASSURANCE-QS-SOURCE-PICKER-V03-20260602";
  ...
  function injectStyle() {
    if ($("csvbSourcePickerV03Style")) return;
    const style = document.createElement("style");
    style.id = "csvbSourcePickerV03Style";
    style.textContent = `
      #csvbSourcePickerPanel{margin-top:0}.csvb-picker-controls{display:grid;grid-template-columns:260px 1fr 120px auto auto auto;gap:7px;align-items:end;margin:8px 0}.csvb-source-checks{display:flex;gap:10px;flex-wrap:wrap;border:1px solid #cbd7ee;border-radius:9px;min-height:36px;align-items:center;padding:5px 9px;background:#fff}.csvb-source-checks label{display:inline-flex;gap:6px;align-items:center;margin:0;font-weight:450}.csvb-source-checks input{width:auto!important;min-height:auto!important}.csvb-picker-controls label{font-size:.82rem;font-weight:600;margin:0 0 4px;color:#1a4170}.csvb-picker-controls input{width:100%;box-sizing:border-box;border:1px solid #cbd7ee;border-radius:9px;padding:7px 9px;background:#fff;color:#1a4170;font-size:.9rem;min-height:36px}.csvb-picker-meta{font-size:.82rem;color:#48628e;margin:6px 0}.csvb-picker-table-wrap{max-height:470px;overflow:auto;border:1px solid #d5deef;border-radius:10px;background:#fff}.csvb-picker-table{width:100%;border-collapse:collapse;min-width:1040px}.csvb-picker-table th,.csvb-picker-table td{padding:7px 8px;border-bottom:1px solid #e6eefb;vertical-align:top;font-size:.86rem}.csvb-picker-table th{background:#f2f7ff;color:#1a4170;font-weight:650;text-align:left;position:sticky;top:0;z-index:1}.csvb-picker-small{font-size:.8rem;color:#48628e;line-height:1.3}.csvb-picker-dup{opacity:.55;background:#f6f8fb}.csvb-picker-check{width:auto!important;min-height:auto!important}@media(max-width:1200px){.csvb-picker-controls{grid-template-columns:1fr 1fr}.csvb-picker-controls .wide{grid-column:1/-1}}@media(max-width:620px){.csvb-picker-controls{grid-template-columns:1fr}.csvb-picker-table{min-width:900px}}
    `;
  ...
  function ensurePanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="note">Select one or more sources, filter the list, tick questions, then save selected questions to this question set.</div>
      <div class="csvb-picker-controls">
        <div>
          <label>Sources visible</label>
          <div class="csvb-source-checks">
            <label><input type="checkbox" data-picker-source="SIRE_2_0" checked /> SIRE 2.0</label>
            <label><input type="checkbox" data-picker-source="RISQ_3" checked /> RISQ 3</label>
          </div>
        </div>
        ...
        <button class="btn" id="csvbPickerAddBtn" type="button">Save Selected to Question Set</button>
      </div>
    ...
    $("csvbPickerBody")?.addEventListener("change", e => {
      const cb = e.target.closest("input[data-row-key]");
      if (!cb) return;
      if (cb.checked) selectedKeys.add(cb.dataset.rowKey); else selectedKeys.delete(cb.dataset.rowKey);
      updateMeta();
    });
  }
  ...
  async function loadQuestionRowsFromRpc() {
    ...
    const fallback = await sb.rpc("csvb_questions_master_for_me");
  ...
  async function loadSourceRows() {
    ...
    selectedKeys.clear();
    await loadExistingKeys();
    $("csvbPickerMeta").textContent = "Loading source questions...";
    allRows = (await loadQuestionRowsFromRpc())
      .filter(row => Object.keys(SOURCE_MAP).includes(sourceForRow(row)))
      .filter(row => {
        const status = s(row.status).trim().toLowerCase();
        return !status || status === "active";
      })
      .sort((a,b) => sourceForRow(a).localeCompare(sourceForRow(b)) || qno(a).localeCompare(qno(b), undefined, { numeric:true }));
    renderPicker();
  }
  ...
  function filteredRows() {
    ...
    sourceRows = allRows.filter(row => sources.includes(sourceForRow(row)));
    return sourceRows.filter(row => {
      const p = row.payload || {};
      const no = qno(row);
      if (ch && chapter(row) !== ch) return false;
      if (!term) return true;
      const hay = [sourceLabel(sourceForRow(row)), no, pget(p,["short_text","Short Text","shortText"]), pget(p,["question","Question"]), pget(p,["inspection_guidance","Inspection Guidance","guidance"]), pget(p,["guide_to_inspection"]), pget(p,["inspection_marker"])].join(" ").toLowerCase();
  ...
  function renderPicker() {
    ...
    if (!allRows.length) {
      body.innerHTML = `<tr><td colspan="6">No source questions loaded.</td></tr>`;
      updateMeta();
      return;
    }
    if (!visibleRows.length) {
      body.innerHTML = `<tr><td colspan="6">No matching questions.</td></tr>`;
      updateMeta();
      return;
    }
    body.innerHTML = visibleRows.map(row => {
      ...
      const no = qno(row);
      const p = row.payload || {};
      const shortText = pget(p,["short_text","Short Text","shortText","inspection_marker"]);
      const question = pget(p,["question","Question","question_text"]);
      const isDup = existingKeys.has(existingKey(source, no));
      const checked = selectedKeys.has(key) && !isDup;
      return `<tr class="${isDup ? "csvb-picker-dup" : ""}"><td><input class="csvb-picker-check" type="checkbox" data-row-key="${esc(key)}" ${checked ? "checked" : ""} ${isDup ? "disabled" : ""} /></td><td>${esc(sourceLabel(source))}</td><td class="mono">${esc(no)}</td><td>${esc(chapter(row)||"-")}</td><td><div>${esc(shortText || question || "-")}</div>${question && question !== shortText ? `<div class="csvb-picker-small">${esc(question)}</div>` : ""}</td><td>${isDup ? "Already added" : "Available"}</td></tr>`;
    }).join("");
    updateMeta();
  }
  ...
  function updateMeta() {
    ...
    if (!meta) return;
    const dupCount = visibleRows.filter(row => existingKeys.has(existingKey(sourceForRow(row), qno(row)))).length;
    const counts = Object.keys(SOURCE_MAP).map(src => `${sourceLabel(src)}: ${allRows.filter(row => sourceForRow(row) === src).length}`).join(" · ");
    meta.textContent = `${visibleRows.length} visible / ${sourceRows.length} source-filtered / ${allRows.length} loaded. ${counts}. ${selectedKeys.size} selected. ${dupCount} already in this set.`;
  }
  ...
  function selectVisible() {
    for (const row of visibleRows) {
      const source = sourceForRow(row);
      const no = qno(row);
      if (!existingKeys.has(existingKey(source, no))) selectedKeys.add(rowKey(row));
    }
    renderPicker();
  }
  ...
  async function addSelected() {
    ...
    const selected = allRows.filter(row => selectedKeys.has(rowKey(row)));
    ...
    for (const row of selected) {
      const source = sourceForRow(row);
      const no = qno(row);
      const key = existingKey(source, no);
      ...
      payloads.push({
        ...
        section: pget(p,["section","Section","section_title","section_code"]) || null,
        short_text: pget(p,["short_text","Short Text","shortText","inspection_marker"]) || null,
        custom_question_text: pget(p,["question","Question","question_text"]) || null,
        expected_evidence: pget(p,["expected_evidence","Expected Evidence","ExpEv_Bullets"]) || null,
        inspector_guidance: pget(p,["inspection_guidance","Inspection Guidance","guidance","guide_to_inspection"]) || null,
        criticality: "normal",
        is_required: true,
        ...
        metadata: { created_from:"source_picker_v03", build:BUILD }
      });
      nextOrder += 100;
    }
    ...
    if (error) throw error;
    showOk(`${payloads.length} source question(s) saved to question set. Reloading...`);
