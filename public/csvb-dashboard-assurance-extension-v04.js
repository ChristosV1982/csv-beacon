// public/csvb-dashboard-assurance-extension-v04.js
// C.S.V. BEACON — Dashboard Inspection & Assurance extension v04.
// Adds Inspection & Assurance into Platform Area Home and persistently corrects the area module count.
// Display-only dashboard injection. No database writes, no auth changes, no RLS changes.

(() => {
  "use strict";

  const BUILD = "DASHBOARD-ASSURANCE-EXTENSION-V04-20260601";
  ...
  function injectStyle() {
    if (document.getElementById("csvbDashboardAssuranceExtensionV04Style")) return;

    const style = document.createElement("style");
    style.id = "csvbDashboardAssuranceExtensionV04Style";
    style.textContent = `
  ...
  function updateAreaModuleCount() {
    ...
    const countNodes = Array.from(document.querySelectorAll(`[data-platform-area-count="${AREA_KEY}"]`));

    if (!panel || !countNodes.length) return 0;

    const itemCount = panel.querySelectorAll(".csvb-area-home-item").length;
    if (!itemCount) return 0;

    const label = itemCount === 1 ? "1 module" : `${itemCount} modules`;
  ...
  async function applyVisibility() {
    ...
    window.CSVB_DASHBOARD_ASSURANCE_EXTENSION_STATE = {
      ...
      count_text: document.querySelector(`[data-platform-area-count="${AREA_KEY}"]`)?.textContent || "",
    };
  }
  ...
  function start() {
    ...
    [300, 700, 1200, 2000, 3200, 5000].forEach((ms) => {
