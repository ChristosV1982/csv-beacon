...
const BUILD = "assurance_question_set_items_page_v01_20260601";
...
function renderItems(){
  ...
  body.innerHTML=ITEMS.map(row=>{
    const text=row.short_text||row.custom_question_text||row.expected_evidence||"-";
    ...
    return `<tr><td class="mono">${esc(row.item_no||"-")}</td><td>${esc(sourceLabel(row.source_library))}</td><td class="mono">${esc(row.source_question_no||"-")}</td><td><div>${esc(text)}</div><div class="small">${esc(sub||"-")}</div></td><td>${esc(row.criticality||"normal")}</td><td>${pill}</td><td><button class="btn secondary" data-action="toggle" data-id="${esc(row.id)}" ${!canManage()?"disabled":""}>${row.is_active===false?"Activate":"Deactivate"}</button></td></tr>`;
  }).join("");
}
function clearForm(){["questionNo","chapter","section","shortText","questionText","expectedEvidence","guidance"].forEach(id=>{const n=el(id); if(n)n.value=""});el("sourceLibrary").value="SIRE_2_0";el("criticality").value="normal";el("isRequired").checked=true;el("answerRequired").checked=true;el("findingAllowed").checked=true;}
function payload(){
...
async function addItem(){clearMessages();const {error}=await sb.from("assurance_question_set_items").insert(payload());if(error)throw error;showOk("Item added.");clearForm();await loadItems();}
...
function bind(){el("logoutBtn")?.addEventListener("click",async()=>{await window.AUTH.logout()});el("addBtn")?.addEventListener("click",()=>addItem().catch(e=>showWarn(e.message||String(e))));el("clearBtn")?.addEventListener("click",clearForm);el("refreshBtn")?.addEventListener("click",()=>loadItems().catch(e=>showWarn(e.message||String(e))));el("itemsBody")?.addEventListener("click",e=>{const b=e.target.closest("button[data-action]");if(!b)return;toggleItem(b.dataset.id).catch(err=>showWarn(err.message||String(err)));});}
async function init(){try{bind();SESSION=await getSession();PROFILE=await getProfile();await loadQuestionSet();if(!canManage()){el("editorCard").style.display="none";}await loadItems();window.CSVB_INSPECTION_QS_ITEMS_PAGE={build:BUILD,question_set_id:QUESTION_SET.id,item_count:ITEMS.length};}catch(e){showWarn(e.message||String(e));if(el("subLine"))el("subLine").textContent="Not ready."}}
init();
