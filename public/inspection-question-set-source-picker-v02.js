// public/inspection-question-set-source-picker-v02.js
// C.S.V. BEACON - Assurance Question Set Source Picker v02
// Multi-source checkbox picker with bulk save to question set.

(() => {
  "use strict";

  const BUILD = "ASSURANCE-QS-SOURCE-PICKER-V02-20260602";
  ...
  function injectStyle() {
    if ($("csvbSourcePickerV02Style")) return;
    const style = document.createElement("style");
    style.id = "csvbSourcePickerV02Style";
    style.textContent = `
      ...
      <div class="csvb-picker-controls">
        <div>
          ...
          <div class="csvb-source-checks">
            ...
            <label><input type="checkbox" data-picker-source="RISQ_3" /> RISQ 3</label>
          </div>
        </div>
  ...
  function filteredRows() {
    ...
    return sourceRows.filter(row => {
      ...
      if (!term) return true;
      const hay = [sourceLabel(sourceForRow(row)), no, pget(p,["short_text","Short Text","shortText"]), pget(p,["question","Question"]), pget(p,["inspection_guidance","Inspection Guidance","guidance"])].join(" ").toLowerCase();
  ...
  function updateMeta() {
    ...
    meta.textContent = `${visibleRows.length} visible / ${sourceRows.length} source-filtered / ${allRows.length} loaded. ${selectedKeys.size} selected. ${dupCount} already in this set.`;
  }
  ...
  async function addSelected() {
    ...
    for (const row of selected) {
      ...
      payloads.push({
        ...
        metadata: { created_from:"source_picker_v02", build:BUILD }
      });
      nextOrder += 100;
    }
