...
(() => {
  ...
  function sourceDetails(row) {
    ...
    const fields = [
      ...
      detailField("Question type / origin", pget(p,["question_type","Question Type","question_origin"])),
    ...
    const raw = Object.keys(p).length ? `<details class="csvb-picker-raw"><summary>Raw source details</summary><pre>${esc(JSON.stringify(p,null,2))}</pre></details>` : "";
    return `<details class="csvb-picker-more"><summary>More question information</summary><div class="csvb-picker-more-body"><div class="csvb-picker-detail-grid">${fields || detailField("Information", "No additional stored details.")}</div>${raw}</div></details>`;
  }
