// public/csvb-dashboard-assurance-extension-v02.js
// C.S.V. BEACON — Dashboard Inspection & Assurance extension v02.
// Adds the module both to the legacy dashboard grid and to the Platform Area Home renderer.
// Display-only dashboard injection. No database writes, no auth changes, no RLS changes.

(() => {
  "use strict";

  const BUILD = "DASHBOARD-ASSURANCE-EXTENSION-V02-20260601";
  ...
  function injectStyle() {
    if (document.getElementById("csvbDashboardAssuranceExtensionV02Style")) return;

    const style = document.createElement("style");
    style.id = "csvbDashboardAssuranceExtensionV02Style";
    style.textContent = `
  ...
  function injectIntoAreaHomeConfig() {
    ...
    const vetting = config?.inspection_libraries_vetting;
