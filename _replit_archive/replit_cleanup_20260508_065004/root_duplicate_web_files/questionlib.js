...
// Shared helpers for loading the SIRE 2.0 question library and filtering/sorting questions.
...
export function buildFilterOptions(questions) {
  ...
  return {
    ...
    spisRanks: uniq(questions.map(q => q["SPIS Rank Allocation"] ?? q.spis_rank_allocation)),
    companyRanks: uniq(
...
export function applyFilters(questions, filters) {
  ...
  return questions.filter(q => {
    ...
    const spisRank = String(q["SPIS Rank Allocation"] ?? q.spis_rank_allocation ?? "");
    ...
    if (filters.spisRank && spisRank !== String(filters.spisRank)) return false;
